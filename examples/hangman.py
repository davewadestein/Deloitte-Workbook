import random
# Grab a random word from wordlist.txt (which is in this folder)
...
while incorrect_guesses < max_incorrect_guesses:
    ...
    # ...OR a whole word. 
    # First we will compare the user's guess against the word.
    # If it's equal, they won.
    if guess == word:
        print('Congrats, you guessed the word!')
        break
        
    # At this point, whatever they typed was NOT the word. So it
    # is either an incorrect word, or it is a single letter. We
    # check the length, and if it's a word (i.e., length > 1),
    # it must bewrong.
    ...
    # What do we know about guess here? It is a single letter.
    # If that letter is NOT IN the word, then it's a wrong guess
    # and we try again.
    ...
    # At this point, they have guessed a single letter and it IS in the word...
    for position, letter in enumerate(word):
        if guess == letter:
            letter_list[position] = letter
        
# At this point we either exited the loop via a break (which means the user
# got it right), or normally, which means the user did not.
else:
